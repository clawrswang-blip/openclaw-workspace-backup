...
<!-- openclaw-memory-promotion:memory:memory/2026-07-17.md:29:32 -->
- 🔧 执行的修复（4 步）: **bootout LaunchAgent** — `launchctl bootout gui/$UID/com.openai.codex-dream-skin-studio.complete-install`; **删非法 config 字段** — 用 Python 正则安全删除 `dock-icon-preference = "codex-system"`（备份到 `~/.codex/config.toml.bak-pre-dream-cleanup`）; **写 sentinel** — 在 `~/Library/Application Support/CodexDreamSkinStudio/user-disabled.marker` 写入禁用标记; **截断 launchd 日志** — `~/.codex/.codex-dream-skin-bootstrap/launchd.{stdout,stderr}.log` 清空 [score=0.826 recalls=0 avg=0.620 source=memory/2026-07-17.md:29-32]
<!-- openclaw-memory-promotion:memory:memory/2026-07-17.md:38:40 -->
- ✅ 验证: Codex 桌面 App 启动后所有进程（ChatGPT 主进程、Codex (Service)、Codex (Renderer)、SkyComputerUseService、codex_chronicle）稳定运行 30+ 秒，CPU < 2%; `launchctl list` 中无 dream skin 残留; 无 startup crash [score=0.826 recalls=0 avg=0.620 source=memory/2026-07-17.md:38-40]
<!-- openclaw-memory-promotion:memory:memory/2026-07-17.md:4:4 -->
- 🎯 问题: Rishon 报告 Codex 桌面 App 一直在闪退。CLI 部分之前已修了 skill description 和版本升级，但桌面 App（ChatGPT.app 集成的 Codex）仍在闪退。 [score=0.826 recalls=0 avg=0.620 source=memory/2026-07-17.md:4-4]
<!-- openclaw-memory-promotion:memory:memory/2026-07-16.md:5:7 -->
- Status Check: **Rishon 活跃状态:** 沉默第 63 天（最后活跃 2026-05-14）; **今日会话:** 仅心跳监控，无实际消息; **新内容:** 无 [score=0.812 recalls=0 avg=0.620 source=memory/2026-07-16.md:5-7]
