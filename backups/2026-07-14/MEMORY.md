...
| 🆕 2026-07-05 | finesse-ui | 高端界面制作skill（mouse-lin/finesse-skill）；**所有 HTML 制作强制标准**；brand（落地页/品牌站）+ product（仪表盘/后台）两路；Design Read + 三档旋钮 + 高级感物理层 + 反廉价黑名单；仓库：`https://github.com/mouse-lin/finesse-skill` |
...
## 🆕 2026-07-09 productivity Skills 安装（Matt Pocock）
- 来源：https://github.com/mattpocock/skills
- 路径：`skills/mattpocock/productivity/`（5个）
- 子 skills：
  - `grill-me` — 触发 grilling session（user-invoked）
  - `grilling` — relentless interview 直到决策树每个分支都被覆盖
  - `handoff` — 将会话压缩为handoff文档给下一个agent继续（user-invoked）
  - `teach` — 多session状态化学期 workspace（user-invoked）
  - `writing-great-skills` — skill写作规范参考（user-invoked）
...
## 🆕 2026-07-06 cheat-on-content 安装
- 来源：XBuilderLAB/cheat-on-content (GitHub)
- 路径：`skills/cheat-on-content/`（symlink）
- 性质：**内容创作预测闭环系统** — 打分→盲预测→发布→T+3d复盘→进化rubric
- 触发词（按场景调用）：
  - `初始化` → cheat-init（首次使用）
  - `找对标`/`learn from` → cheat-learn-from
  - `打分这篇` → cheat-score
  - `启动预测` → cheat-predict
  - `拍了`/`shot it` → cheat-shoot
  - `已发布` → cheat-publish
  - `复盘`/`retro` → cheat-retro
  - `升级rubric` → cheat-bump
  - `推荐选题` → cheat-recommend
  - `抓热点` → cheat-trends
  - `状态`/`status` → cheat-status
- **⚠️ 首次使用：必须在内容项目目录运行 `初始化`**
- **social media 相关内容制作时自动调用此 skill**

## Promoted From Short-Term Memory (2026-07-13)

<!-- openclaw-memory-promotion:memory:memory/2026-07-08.md:12:15 -->
- DashiAI PPT Skill 安装: 媒体素材管理（image-gen / 本地上传）; **工作流**: 用户需求 → JSON plan → render_goal_deck.sh → index.html + assets; **Node 依赖**: v24.14.0 ✅（已内置 node_modules）; **注意**: 渲染脚本默认端口 5200-5999 段；PPTX 导出需要本机 HTTP 预览服务 [score=0.893 recalls=0 avg=0.620 source=memory/2026-07-08.md:12-15]
<!-- openclaw-memory-promotion:memory:memory/2026-07-08.md:4:7 -->
- DashiAI PPT Skill 安装: **时间**: 2026-07-08 09:36 PDT; **来源**: https://github.com/chuspeeism/dashiai-ppt-skill; **安装路径**: `~/.openclaw/workspace/skills/dashiai-ppt/`; **触发词**: PPT、演示文稿、幻灯片、汇报材料 [score=0.893 recalls=0 avg=0.620 source=memory/2026-07-08.md:4-7]
<!-- openclaw-memory-promotion:memory:memory/2026-07-08.md:8:11 -->
- DashiAI PPT Skill 安装: **核心能力**:; 12种视觉主题（theme01-12）; 生成 HTML PPT（可离线打开、浏览器编辑）; 导出 PPTX / PDF [score=0.893 recalls=0 avg=0.620 source=memory/2026-07-08.md:8-11]
<!-- openclaw-memory-promotion:memory:memory/2026-07-09.md:13:13 -->
- 视频创作 Skill 强制绑定规则: TOOLS.md 新增「视频创作 Skill 绑定规则」章节： [score=0.861 recalls=0 avg=0.620 source=memory/2026-07-09.md:13-13]
<!-- openclaw-memory-promotion:memory:memory/2026-07-09.md:20:20 -->
- Rishon 的关键项目备注: Rishon 提到他要做"导演 skill"相关工作，具体需求待进一步明确。已安装好基础设施。 [score=0.861 recalls=0 avg=0.620 source=memory/2026-07-09.md:20-20]
<!-- openclaw-memory-promotion:memory:memory/2026-07-09.md:9:10 -->
- seedance-2.0 导演 Skill 安装: 参考文档：30+ 个（directing-engine / capability-map / allocation-model / pro-filmmaking-standards 等）; 性质：**四模态AI电影制作完整 pipeline**（文本/图片/视频/参考） [score=0.861 recalls=0 avg=0.620 source=memory/2026-07-09.md:9-10]
<!-- openclaw-memory-promotion:memory:memory/2026-07-09.md:5:8 -->
- seedance-2.0 导演 Skill 安装: 来源：https://github.com/Emily2040/seedance-2.0; 安装路径：`skills/emily-seedance/`（git clone）; 版本：v6.6.0; 子 skill 数量：28 个（seedance-prompt / seedance-sequence / seedance-continuation / seedance-camera / seedance-lighting / seedance-motion / seedance-vfx / seedance-recipes / seedance-style 等） [score=0.861 recalls=0 avg=0.620 source=memory/2026-07-09.md:5-8]
<!-- openclaw-memory-promotion:memory:memory/2026-07-09.md:14:17 -->
- 视频创作 Skill 强制绑定规则: 导演层：seedance-2.0（理解创意→Shot List→Prompt设计→质量把控）; 执行层：tvc-director（宫格图→视频脚本→Hailuo生成）; 生成层：video_generate（MiniMax Hailu API）; 决策链：seedance-2.0 → tvc-director → video_generate → retake-protocol [score=0.861 recalls=0 avg=0.620 source=memory/2026-07-09.md:14-17]
