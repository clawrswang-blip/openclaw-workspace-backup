...
## 每周一：战略检查（通过 WhatsApp 发送，不在深夜打扰）
- Sungiven 会员数据：上周增长情况，离 10 万目标还有多远？
- 国内 AI 咨询公司：新项目进展？信誉楼有新消息吗？
- OpenClaw：有没有值得记录的使用心得或迭代方向？
- 周 PM 汇总：改通过 WhatsApp 发送（不在 4am 打扰）

## 每日（可选）：
- Rishon 有没有给我新的方向或向量？（如果是，记录到 memory/YYYY-MM-DD.md）
- Rishon 最近问了什么新问题？（可以帮助我更新上下文）

## 🔍 跨 Session 活动感知（新增 2026-06-22）
**问题背景：** Rishon 在 subagent / webchat / isolated session 里的对话不会出现在 WhatsApp 主 session 里，我的心跳逻辑需要主动扫描。

**扫描命令：**
```bash
python3 ~/.openclaw/workspace/scripts/session_activity_scanner.py 2
```

**检查逻辑：**
1. 运行 scanner，扫描最近 2 天内的所有 session 文件
2. 如果发现非 cron/heartbeat 的真实用户消息 → 说明 Rishon 有活动但我没感知到
3. 如果活动量 > 5 条 → 更新 memory/YYYY-MM-DD.md，记录"感知盲区已修复"
4. 如果发现 FAILED session → 写入 postmortem/logs/

**注意：** scanner 会自动过滤 cron 消息和 heartbeat poll，不需要人工判断。
...
## 🔄 PM 触发检查（每次心跳执行）
**检查逻辑：**
1. 读取 `postmortem/logs/{today}.md` 的最后一条 PM 记录
2. 如果距上次 PM 已 >6 小时，且期间 tool_calls ≥ 10 → 触发 PM 提醒
3. 如果任务 FAILED → 立即触发 PM，不等

**PM 提醒格式：**
当检测到需要 PM 时，在心跳响应末尾加上：
```
📋 PM 提醒：你刚才有一个复杂任务（{n} tool calls）还没填 PM。
需要我帮你记录吗？格式如下：

PM:{date}|{session_id}|{task_type}|{tool_count}|{outcome}
PATTERNS:...
CONTEXT:...
JUDGMENT:...
ACTIONS:...
```
**注意：** PM 不是给 Rishon 填的——是 Luna 自己填的。如果 Rishon 确认不需要，就不填。
