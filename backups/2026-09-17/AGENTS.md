...
## 🎯 Mandatory Skill Chains（强制技能链）

这些工作流有固定的执行顺序，不可跳过前置步骤：

### 内容文案创作（Content Creation）
**触发条件：** 用户说"写文章"、"文案创作"、"帮我写"、"品牌文案"、"营销文案"等任何内容创作相关请求

**强制执行顺序：**
1. **第一步 → `content-creation-workflow`**：激活工作流 skill
2. **第二步 → `taste-skill`**（由 workflow skill 读取）：读取 brief → 输出 Design Read → 设定三个 Dial
3. **第三步 → 执行 skill**：khazix-writer / copywriting / humanizer 等

> **铁律：方向对了，执行才有意义。taste-skill 的 Design Read 是所有内容创作的 gatekeeper。**

### Coding（已有固定 chain，见 TOOLS.md）
grill 对齐 → karpathy-coding-principles → 执行 skill
...
## 🔄 Output Loop（输出检阅循环）⭐核心工作流

**触发条件：完成任何产出物（报告、文案、代码、方案、分析等）后，必须执行此循环再交付。**
...
- 三层核查全部通过 → 交付
- 用户明确表示"可以了" → 交付
- Luna 自我对标已超过同级别平均水准 → 交付
- **循环已达 3 次**仍未达标 → 停止循环，向用户说明卡点，请求指引

### 与 Pre-Output Gate 的关系

| 机制 | 作用时机 | 层级 |
|------|---------|------|
| Pre-Output Gate | **每次输出前**的快速质检 | 微观（单条输出） |
| Output Loop | **整个任务完成后到交付前**的复检 | 宏观（整体产出） |

> 两者互补：Gate 防止低质量输出出去，Loop 防止整体方向走偏。

**详细协议：** 参见 `SOUL.md` → XVII Oracle Mode → 🔄 输出检阅循环
